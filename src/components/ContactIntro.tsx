    ...
    <section className="bg-white pt-24 md:pt-36 pb-6 md:pb-14">
      <div className="max-w-screen-2xl mx-auto px-6 lg:px-12">
        <motion.div
          ...
          className="grid grid-cols-1 lg:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)] gap-6 lg:gap-16 items-end"
        >
          <div>
            ...
            <h1 className="font-display text-4xl sm:text-5xl md:text-6xl xl:text-7xl font-bold uppercase tracking-tighter leading-[0.95] mb-4 sm:mb-6">
          ...
          {/* No box: the contact block directly below is already a bordered
              grid, and a second frame here made the top of the page read as two
              competing cards. A rule and hairline-divided rows carry the same
              grouping with less furniture. */}
          <div className="border-t border-brand-900 pt-5 sm:pt-6">
            <p className="font-display font-bold uppercase text-[10px] sm:text-xs text-gray-500 mb-2 sm:mb-3 tracking-[0.2em]">
              What you get
            </p>
            <ul className="divide-y divide-brand-900/10">
              {INCLUDED.map((item) => (
                <li key={item} className="flex items-start gap-3 py-3 sm:py-3.5">
